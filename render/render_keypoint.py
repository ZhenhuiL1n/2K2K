...
parser.add_argument(
    "--data_path",
    type=str,
    default="/workspace/dataset/data/",
    help="path to mesh data",
)

parser.add_argument(
    "--data_name", type=str, default="RP", help="folder name of rendered dataset"
)
parser.add_argument(
    "--smpl_model_path", type=none_or_str, default="None", help="path to smplx model"
)
parser.add_argument(
    "--render_ORTH", type=bool, default=False, help="path to smplx model"
)
...
def keypoint_projection(
    ...
    if data_name == "THuman2":
        ...
        )  # mesh path '/workspace/dataset/RenderPeople/'
    joint_path = os.path.join(
        data_path, "Joint3D", data_name
    )  # '/workspace/dataset/RP_2048/Joint3D/'
    proj_result_path = os.path.join(
        data_path, "PERS", "keypoint"
    )  # '/workspace/dataset/RP_2048/PERS/keypoint/'
    pers_path = os.path.join(
        data_path, "PERS", "COLOR", "NOSHADING"
    )  # '/workspace/dataset/RP_2048/PERS/COLOR/NOSHADING/'
    sample_path = os.path.join(
        data_path, "PERS", "keypoint_sample"
    )  #'/workspace/dataset/RP_2048/PERS/keypoint_sample/'
    x_angles = list(
        range(angle_min_x, angle_max_x + 1, interval_x)
    )  # [-30, -20, -10, 0, 10, 20, 30]
    ...
    for angle in x_angles:
        if not os.path.isdir(
            os.path.join(proj_result_path, data_name + "_0_y_{}_x".format(angle))
        ):
            os.makedirs(
                os.path.join(proj_result_path, data_name + "_0_y_{}_x".format(angle))
            )
        if not os.path.isdir(
            os.path.join(sample_path, data_name + "_0_y_{}_x".format(angle))
        ):
            os.makedirs(
                os.path.join(sample_path, data_name + "_0_y_{}_x".format(angle))
            )
    ...
    for d in tqdm(data):
        ...
        if data_name == "THuman2":
            glob_rotation = np.array([0.0, 0.0, 0.0], dtype=np.float32)
        ...
        for angle in x_angles:
            ...
            img_path = os.path.join(
                pers_path, data_name + "_0_y_{}_x".format(angle), d[:-3] + "front.png"
            )
            ...
            if data_name == "RP":
                save_path = os.path.join(
                    proj_result_path,
                    data_name + "_0_y_{}_x".format(angle),
                    d[:-4] + "_front.npy",
                )
            elif data_name == "THuman2":
                save_path = os.path.join(
            ...
            if data_name == "RP":
                ...
                img_path = os.path.join(
                    pers_path, data_name + "_0_y_{}_x".format(angle), d + "_front.png"
                )
            ...
            for k in range(cam_pc.shape[0]):
                ...
                if k in [23, 24, 25, 26]:
                    img_cv = cv2.circle(
                        img_cv, (x, y), radius=0, color=(255, 0, 0), thickness=int(3)
                    )
                elif k in [27, 28, 29, 30]:
                    img_cv = cv2.circle(
                        img_cv, (x, y), radius=0, color=(0, 255, 0), thickness=int(3)
                    )
                else:
                    img_cv = cv2.circle(
                        img_cv, (x, y), radius=0, color=(0, 0, 255), thickness=int(3)
                    )
            if data_name == "RP":
                sam_path = os.path.join(
                    sample_path, data_name + "_0_y_{}_x".format(angle), d[:-4] + ".png"
                )
            elif data_name == "THuman2":
                sam_path = os.path.join(
                    sample_path, data_name + "_0_y_{}_x".format(angle), d + ".png"
                )
...
def param_to_tensor(param, device):
    for key in param.keys():
        try:
            ...
            param[key] = torch.reshape(
                torch.as_tensor(param[key], device=device), (1, -1)
            )
...
def refine(d, keypoint, smpl_joint=None):
    ...
    if d[:-4] == "rp_alvin_posed_016":
        keypoint["joint3d"][8] = np.array(
            [1.0385541517671586, 143.8048885415659, 20.073939913489461]
        )
        ...
        keypoint["joint3d"][15] = np.array([4.0, 3.0, 4.0])
        keypoint["joint3d"][19] = np.array([4.0, 3.0, 4.0])
        keypoint["joint3d"][16] = np.array([-13.0, 3.0, 4.0])
        keypoint["joint3d"][22] = np.array([-13.0, 3.0, 4.0])
        keypoint["joint3d"][17] = np.array([4.0, 3.0, 20.0])
        keypoint["joint3d"][18] = np.array([4.0, 3.0, 20.0])
        keypoint["joint3d"][20] = np.array([-13.0, 3.0, 20.0])
        keypoint["joint3d"][21] = np.array([-13.0, 3.0, 20.0])
        keypoint["visible"][[15, 19, 16, 22, 17, 18, 20, 21]] = 1
    elif d[:-4] == "rp_holly_posed_010":
        keypoint["joint3d"][7] = np.array([12.0, 150.0, -0.8])
        ...
        keypoint["joint3d"][8] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][10] = np.array([34.0, 155.0, -11.0])
        keypoint["joint3d"][96] = np.array([16.0, 149.0, 6.0])
        keypoint["joint3d"][100] = np.array([16.0, 149.0, 6.0])
        keypoint["joint3d"][104] = np.array([16.0, 149.0, 6.0])
        keypoint["joint3d"][108] = np.array([16.0, 149.0, 6.0])
        keypoint["joint3d"][117] = np.array([44.0, 155.0, -16.0])
        keypoint["joint3d"][121] = np.array([44.0, 155.0, -16.0])
        keypoint["joint3d"][125] = np.array([44.0, 155.0, -16.0])
        keypoint["joint3d"][129] = np.array([44.0, 155.0, -16.0])
        keypoint["visible"][[8, 10, 96, 100, 104, 108, 117, 121, 125, 129]] = 1
    elif d[:-4] == "rp_tenzin_posed_001":
        keypoint["joint3d"][0] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][1] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][2] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][3] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][4] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][8] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][20] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][21] = np.array([14.0, 149.0, 4.0])
        keypoint["joint3d"][22] = np.array([14.0, 149.0, 4.0])
    ...
    # [96, 100, 104, 108, 117, 121, 125, 129]
        ...
        # 117, 121, 125, 129
        keypoint["joint3d"][96] = smpl_joint[
            0, 67, :
        ].cpu().detach().numpy() + np.array([0, -0.028, 0])
        keypoint["joint3d"][100] = smpl_joint[
            0, 68, :
        ].cpu().detach().numpy() + np.array([0, -0.028, 0])
        keypoint["joint3d"][104] = smpl_joint[
            0, 69, :
        ].cpu().detach().numpy() + np.array([0, -0.028, 0])
        keypoint["joint3d"][108] = smpl_joint[
            0, 70, :
        ].cpu().detach().numpy() + np.array([0, -0.028, 0])
        ...
        keypoint["joint3d"][20] = keypoint["joint3d"][16] + np.array(
            [0.02, -0.02, -0.05]
        )
        keypoint["joint3d"][21] = keypoint["joint3d"][16] + np.array(
            [0.02, -0.02, -0.05]
        )
        keypoint["joint3d"][22] = keypoint["joint3d"][16]
    ...
    if smpl_joint is not None:
        ...
        if keypoint["visible"][11] == 0:
            keypoint["joint3d"][11] = (
                smpl_joint[0, 1, :].cpu().detach().numpy()
            )  # 3? 1?
        if keypoint["visible"][12] == 0:
            keypoint["joint3d"][12] = (
                smpl_joint[0, 2, :].cpu().detach().numpy()
            )  # 7? 2?
