...
parser.add_argument("--log_freq", type=int, default=100, help="num epoch to train")
parser.add_argument("--start_epoch", type=int, default=1, help="# to the first epoch")
parser.add_argument(
    "--logs_dir", type=str, default="./logs", help="path to tensorboard"
)
parser.add_argument(
    "--local_rank", type=int, default=[0,1], metavar="N", help="Local process rank."
...
parser.add_argument(
    ...
    type=bool,
    default=True,
    help="indicate whether the currnent is master",
)
...
#     "--load_ckpt",
#     type=str,
#     default="ckpt_alldataset_phase2.pth.tar",
...
parser.add_argument(
    "--data_path",
    type=str,
    default="/home/mbzuai/Storage/Datasets/2K2K_Train/data",
...
parser.add_argument(
    ...
    type=str,
    default="./checkpoints/save_path/",
    help="path to save checkpoints",
)
parser.add_argument(
    "--exp_name", type=str, default="AllData", help="checkpoint name to be saved"
)
...
def train(
    ...
    with tqdm(enumerate(data_loader)) as pbar:
        for i, datum in pbar:
            ...
            image, front_depth, back_depth, mask, init_affine, data_path = (
                dataset.fetch_output(datum)
            )

            # initialize variables (in case of multiple images, they are returned as a tuple).
            image, front_depth, back_depth, mask, init_affine = init_variables(
                image, front_depth, back_depth, mask, init_affine, device=device
            )

            # compute and update losses.
            loss, losses, input_var, pred_var, target_show = loss_builder.build_loss(
                model,
                image,
                front_depth,
                back_depth,
            ...
            if is_master:
                pbar.set_description(
                    "[{0}][{1}/{2}] loss: {loss:.4f}, "
                    "lossN: {lossN:.4f}, lossD: {lossD:.4f}, lossC: {lossC:.4f}, lossM: {lossM:.4f}, lr: {lr:0.10f}".format(
                        epoch,
                        i,
                        iters,
                        loss=loss_batch.val,
...
def main():
    ...
    scheduler = CosineAnnealingWarmUpRestarts(
        optimizer, T_0=5, T_warmup=0.01, decay=0.5
    )
    scheduler.step(args.start_epoch - 1)

    # load checkpoint if required
    
    print("load ckpt: True / False ? ", args.load_ckpt)
    if args.load_ckpt and args.is_master:
        ckpt = torch.load(
            args.checkpoints_load_path + args.load_ckpt
        )  # model : single / ckpt : single & multi
