...
parser.add_argument(
    "--load_ckpt", type=str, default="ckpt_bg_mask.pth.tar", help="somewhere in your PC"
)
...
parser.add_argument(
    "--save_path", type=str, default="./result", help="path to save folder"
)
parser.add_argument(
    "--save_name", type=str, default="test", help="name of save folder inside save_path"
)
...
def main():
    ...
    os.makedirs(
        args.save_path + "/" + args.save_name + "/output_plys_c/", exist_ok=True
    )
    ...
    transform_human = transforms.Compose(
        [
            ...
            transforms.ColorJitter(
                brightness=(0.85, 0.85), contrast=(0.85, 0.85), saturation=(0.85, 0.85)
            ),
            transforms.ToTensor(),
        ]
    )
    ...
    data_list = [
        x for x in os.listdir(data_path) if ".png" in x or ".jpg" in x or ".JPG" in x
    ]
    data_list.sort()
    ...
    with torch.no_grad():
        for f_i_name in tqdm(data_list):
            ...
            start = time.time()
            img_rembg = remove(
                image, post_process_mask=True, session=new_session("u2net")
            )
            ...
            image, _, _, _, init_affine = init_variables(
                image, None, None, None, init_affine, device=args.device
            )
            ...
            p_d_f_down = (
                p_d_f_down[0].detach().cpu().numpy().transpose(1, 2, 0) * 512 * 32
            )
            p_d_f_down = p_d_f_down.astype(np.uint16)
            p_d_b_down = (
                p_d_b_down[0].detach().cpu().numpy().transpose(1, 2, 0) * 512 * 32
            )
            p_d_b_down = p_d_b_down.astype(np.uint16)

            p_d_f_down_mask = (
                p_d_f_down_mask[0].detach().cpu().numpy().transpose(1, 2, 0)
            )
            ...
            corr_depth10 = (
                5 * front_depth[:, :, 0].astype(np.float64) / 6
                + corr1_depth.astype(np.float64) / 6
            )
            corr_depth10[mask01] = 0.0
            corr_depth10 = np.expand_dims(corr_depth10, axis=2)

            corr_depth20 = (
                3 * front_depth[:, :, 0].astype(np.float64) / 5
                + 2 * corr2_depth.astype(np.float64) / 5
            )
            corr_depth20[mask12] = 0.0
            corr_depth20 = np.expand_dims(corr_depth20, axis=2)

            corr_depth30 = (
                front_depth[:, :, 0].astype(np.float64) / 4
                + 3 * corr3_depth.astype(np.float64) / 4
            )
            corr_depth30[mask23] = 0.0
            corr_depth30 = np.expand_dims(corr_depth30, axis=2)

            corr_depth05 = (
                6.5 * front_depth[:, :, 0].astype(np.float64) / 7
                + 0.5 * back_depth[:, :, 0].astype(np.float64) / 7
            )
            corr_depth05[mask_10] = 0.0
            corr_depth05 = np.expand_dims(corr_depth05, axis=2)

            corr_depth15 = (
                4.5 * front_depth[:, :, 0].astype(np.float64) / 6
                + 1.5 * corr1_depth.astype(np.float64) / 6
            )
            corr_depth15[mask01] = 0.0
            corr_depth15 = np.expand_dims(corr_depth15, axis=2)

            corr_depth25 = (
                2.5 * front_depth[:, :, 0].astype(np.float64) / 5
                + 2.5 * corr2_depth.astype(np.float64) / 5
            )
            corr_depth25[mask12] = 0.0
            corr_depth25 = np.expand_dims(corr_depth25, axis=2)

            corr_depth35 = (
                0.5 * front_depth[:, :, 0].astype(np.float64) / 4
                + 3.5 * corr3_depth.astype(np.float64) / 4
            )
            ...
            if image_front.shape[0] != 2048 or image_front.shape[1] != 2048:
                image_front = cv2.resize(
                    image_front, (2048, 2048), interpolation=cv2.INTER_CUBIC
                )
            if image_back.shape[0] != 2048 or image_back.shape[1] != 2048:
                image_back = cv2.resize(
                    image_back, (2048, 2048), interpolation=cv2.INTER_CUBIC
                )
            ...
            xyz_f, rgb_f = pers2pc(
                image_front, front_depth.astype(np.float64) / 32.0, 2048, 50
            )
            xyz_b, rgb_b = pers2pc(
                image_back, back_depth.astype(np.float64) / 32.0, 2048, 50
            )
            ...
            if res == 2048:
                xyz_c1, rgb_c1 = pers2pc(
                    None, corr_depth10.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c2, rgb_c2 = pers2pc(
                    None, corr_depth20.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c3, rgb_c3 = pers2pc(
                    None, corr_depth30.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c05, rgb_c05 = pers2pc(
                    None, corr_depth05.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c15, rgb_c15 = pers2pc(
                    None, corr_depth15.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c25, rgb_c25 = pers2pc(
                    None, corr_depth25.astype(np.float64) / 32.0, 2048, 50
                )
                xyz_c35, rgb_c35 = pers2pc(
                    None, corr_depth35.astype(np.float64) / 32.0, 2048, 50
                )
...
def center_padding(img, pose, set_size):
    ...
    if h < w:
        ...
        right = (h - w) - (h - w) // 2
        img = cv2.copyMakeBorder(
            img, 0, 0, left, right, cv2.BORDER_REPLICATE
        )  # top, bottom, left, right
        pose[:, 0] += left
